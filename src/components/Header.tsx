    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <Link href="/" className="relative z-10 flex items-center group">
          ...
          <span className="ml-3 text-2xl font-black tracking-tighter text-black">
            ANTIGRAVITY<span className="text-red-600 italic">.</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-6">
          ...
          {!loading ? (
            user ? (
              <div className="flex items-center gap-4">
                ...
                >
                  <div className="text-right hidden sm:block">
                    <p className="text-[10px] font-black uppercase text-gray-300 group-hover:text-red-600 transition-colors">
                      Account
                    </p>
                    <p className="text-xs font-black text-black tracking-tight line-clamp-1">
                      {user.name}
                    </p>
                  </div>
              ...
              <Link
                href="/login"
                className="sm:block hidden bg-black text-white px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition-all shadow-xl shadow-black/10 active:scale-95"
              >
                Sign In
              </Link>
            )
      ...
      <div
        className={`md:hidden absolute top-full left-0 right-0 bg-white border-t border-gray-50 transition-all duration-500 overflow-hidden ${
      ...
      >
        <nav className="flex flex-col items-center gap-8 px-4 text-center">
          ...
          { user && (
          <div className="sm:block flex gap-8 pt-8 border-t border-gray-100 w-full justify-center">
            <Link
              href="/wishlist"
              onClick={() => setIsMobileMenuOpen(false)}
              className="text-gray-300 hover:text-red-600 flex flex-col items-center gap-2"
            >
              <svg
                className="w-8 h-8"
                fill="none"
            ...
            <Link
              href="/cart"
              onClick={() => setIsMobileMenuOpen(false)}
              className="text-gray-300 hover:text-black flex flex-col items-center gap-2"
            >
              <svg
                className="w-8 h-8"
                fill="none"
